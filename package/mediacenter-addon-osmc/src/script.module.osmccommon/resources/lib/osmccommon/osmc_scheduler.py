...
class SimpleScheduler(object):
    ...
    def set_trigger(self, right_now):
        ...
        if self.frequency == 1:
            ...
            # End of this current month plus or minus the number of days the user has chosen
            # in settings

            month = max([1, (right_now.month + 1) % 13])
            year = right_now.year if month != 1 else right_now.year + 1

            trigger_day = right_now.replace(year=year, month=month, day=1) + \
                          datetime.timedelta(days=self.daynum - 1)
            # today, with day replaced by 1 and adding 1 to the month plus the number of days
            # the user has chosen minus an extra day to rebase it to month-end
    ...
    def set_trigger_time(self, trigger_time):
        ...
        if self.specific_time:
            ...
            new_trigger = trigger_time.replace(hour=random.randint(0, 23),
    ...
    def step_trigger(self):
        ...
        if self.frequency == 1:
            ...
            if self.daynum > 0:
                ...
                self.trigger_time = \
                    self.trigger_time.replace(year=year, month=month, day=self.daynum)

            else:
                # if the daynum is negative, that is, the user wants the update to run a certain
                # number of days BEFORE month-end, then jump to the first day of the month two
                # months ahead of the current one, and then move back one day to get next months
                # month-end date, then subtract the number of days the user has chosen
                ...
                self.trigger_time = self.trigger_time.replace(year=year, month=month, day=1) + \
                                    datetime.timedelta(days=self.daynum - 1)
...
def test(settings=None):
    ...
    for increment in range(3700):
        right_now += datetime.timedelta(days=1)

        print(increment)
        print(right_now)
        scheduler = SimpleScheduler(x_set, right_now)
