...
class Compressor:
    ...
    def compress_addons(self):
        ...
        for addon in source_directory:
            ...
            if is_addon_dir(self.addon_path):
                ...
                if addon_xml_exists:
                    if REPO_PYTHON_VERSION == 3 and not is_py3(self.addon_xml):
                        continue

                    if (REPO_PYTHON_VERSION == 2 and not PYTHON3_COMPRESS_ONLY and
                            is_py3(self.addon_xml)):
                        continue
    ...
    def create_compressed_addon_release(self):
        ...
        if ((is_unofficial(self.xml_file) and UNOFFICIAL_COMPRESS_ONLY) or
                (REPO_PYTHON_VERSION != 3 and is_py3(self.addon_xml) and PYTHON3_COMPRESS_ONLY)):
            return
    ...
    def _read_version_number(self):
        ...
        if is_py3(self.addon_xml) and '+matrix' not in addon_version and MATRIX_BUILD_VERSION:
...
if __name__ == '__main__':
    ...
    REPO_PYTHON_VERSION = int(CONFIG.get('python_version'))
    ...
    print('Repository Python Version: %d' % REPO_PYTHON_VERSION)
    print(' ')
